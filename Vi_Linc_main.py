...
def run_dbscan(points, pic_dict, min_or_maj, max_cluster, min_cluster, target_clusters=20, verbose=False):
    ...
    if min_or_maj == 'Min':
        hdbscan = HDBSCAN(min_cluster_size=min_cluster, max_cluster_size=max_cluster)
        ...
        if verbose:
            if num_clusters > 0:
                avg_size = sum(cluster_sizes) / num_clusters
                min_size = min(cluster_sizes)
                max_size = max(cluster_sizes)
                print('Minority Class')
                print(f"Number of clusters found: {num_clusters}")
                print(f"Average number of points per cluster: {avg_size:.2f}")
        ...
        # ============================================================================================
        # STEP 1 — EXTREME PROBE
        # ============================================================================================
        if verbose:
            print("=== Step 1: Extreme probe ===")
        ...
        for ms in test_sizes:
            clusterer = HDBSCAN(min_cluster_size=ms)
            labels = clusterer.fit_predict(points_array)
            n_clusters = len(set(labels) - {-1})

            if verbose:
                print(f"ms={ms}: {n_clusters} clusters (target={target_clusters})")

            cluster_counts.append((ms, n_clusters))

            diff = abs(target_clusters - n_clusters)
        ...
        collapse_point = None
        for ms, nc in cluster_counts:
            if nc <= target_clusters:
                collapse_point = ms
                break
        if collapse_point is None:
            collapse_point = max(10, N // 10)

        # ============================================================================================
        # STEP 2 — COARSE GRID SEARCH (with early stopping)
        # ============================================================================================
        if verbose:
        ...
        step = max(2, (end - start) // 100)

        worse_counter = 0
        last_diff = best_difference

        for ms in range(start, end + 1, step):
            clusterer = HDBSCAN(min_cluster_size=ms)
            labels = clusterer.fit_predict(points_array)
            n_clusters = len(set(labels) - {-1})
            diff = abs(target_clusters - n_clusters)
            ...
            if verbose:
                print(f"ms={ms}: {n_clusters} clusters (diff={diff})")
            ...
            if worse_counter >= 2:
                if verbose:
                    print("Early stop: diff worsened twice in a row (Step 2).")
                break
        ...
        # ============================================================================================
        # STEP 3 — LOCAL REFINEMENT (with early stopping)
        # ============================================================================================
        if verbose:
            print("\n=== Step 3: Local refinement ===")

        best_ms = best_model.min_cluster_size
        worse_counter = 0
        last_diff = best_difference

        for ms in range(max(2, best_ms - step), best_ms + step + 1):
            clusterer = HDBSCAN(min_cluster_size=ms)
            labels = clusterer.fit_predict(points_array)
            n_clusters = len(set(labels) - {-1})
            diff = abs(target_clusters - n_clusters)
            ...
            if worse_counter >= 2:
                if verbose:
                    print("Early stop: diff worsened twice in a row (Step 3).")
                break
        ...
        # ============================================================================================
        # STEP 4 — MAX_CLUSTER_SIZE TUNING (with early stopping)
        # ============================================================================================
        if verbose:
            print("\n=== Step 4: max_cluster_size tuning ===")

        if best_num_clusters < target_clusters:

            max_cluster_size = max(10, int(np.ceil(1.2 * N / target_clusters)))
            step_max = max(1, int(0.1 * max_cluster_size))

            worse_counter = 0
            last_diff = best_difference

            while max_cluster_size > 2:
                clusterer = HDBSCAN(
                    min_cluster_size=best_model.min_cluster_size,
                ...
                if verbose:
                    print(f"max_cluster_size={max_cluster_size}: {n_clusters} clusters (diff={diff})")
                ...
                if worse_counter >= 2:
                    if verbose:
                        print("Early stop: diff worsened twice in a row (Step 4).")
                    break
                ...
                max_cluster_size -= step_max

        # ============================================================================================
        # FINAL RESULT — COMPUTE MEANS ONLY
        # ============================================================================================
        labels = best_model.fit_predict(points_array)
        ...
        # Compute means for return value
        cluster_mean_list = {}
        for cid, idxs in clusters.items():
            pts = points_array[idxs]
            cluster_mean_list[cid] = np.mean(pts, axis=0)
...
def special_img_patterns(neighbor_minority, neighbor_majority, img_dict, start_id_lib, min_cluster, max_cluster):
    ...
    emb_min = run_dbscan(emb_min, min_image_lib, min_or_maj='Min', max_cluster=max_cluster, min_cluster=min_cluster)
    emb_maj = run_dbscan(emb_maj, maj_image_lib, min_or_maj='Maj', max_cluster=max_cluster, min_cluster=min_cluster,
                         target_clusters=int(len(emb_min)))
...
def freqsubgraphmining(graph, org_idx, train_nodes, hop, min_class_label, maj_class_label, min_cluster, max_cluster):
    """
    Args:
        g: DGL graph.
        ...
        min_cluster: Minimum amount of points in a cluster.
    ...
    emb_min, emb_maj, wl_to_img = special_img_patterns(neighbor_minority, neighbor_majority, wl_lib,
                                                                       start_id_lib, min_cluster, max_cluster)
...
def add_img_structure(g, node_index, graph_emb_min, graph_emb_maj, img_dict, hop, selection):
    ...
    for n in range(hop, hop + 1):
        ...
        if wl_hash in img_dict:
            ...
            node_emb = get_vit_embedding(img)
...
def ellipticapproach(selection, min_cluster, max_cluster, hop = 1):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(train_test_set)):
        ...
        emb_min, emb_maj, wl_to_img, _ = freqsubgraphmining(g, org_ind, train_indices_list, hop, min_class_label, maj_class_label, min_cluster, max_cluster)
        ...
        eps = 1e-8
        normalized_patternvector = 1 - (patternvector - min_vals) / (max_vals - min_vals + eps)
        ...
        new_features = np.asarray(torch.stack(new_features))
...
if __name__ == '__main__':
    ...
    if data_source == 'Elliptic':
        file = f"Results/Elliptic/VILINC"
    if data_source == 'FraudYelp':
        file = "Results/FraudYelp/VILINC"

    ellipticapproach(selection, min_cluster, max_cluster, hop)
